...
class ModuleGenerator extends Generator
{
    ...
    public function generate()
    {
        ...
        $this->console->line("");
        $this->console->line("Module <comment>[{$name}]</comment> created successfully.");
        $this->console->line("");
        if ($this->console->confirm('Do you want to generate ".gitkeep" files on emty folders?')) {
            $list = $this->getFolders();
            foreach ($list as $key => $value) {
                $path = $this->module->getModulePath($this->getName()).'/'.$value;
                if (count(glob($path.'/*')) === 0 ) {
                    $this->generateGitKeep($path);
                    $this->console->line("<comment>{$value}</comment>: .gitkeep Generated!");
                } else {
                }
            }
            $this->console->line("");
            $this->console->info("All '.gitkeep' files generated on Module [{$name}].");
        }
    }
    ...
    public function generateResources()
    {
        ...
        $this->console->call('module:sidebar-provider', [
            'module' => $this->getName(),
        ]);

        $this->console->call('module:breadcrumbs-provider', [
            'module' => $this->getName(),
        ]);
